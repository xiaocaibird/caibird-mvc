...
let onAppError: ((err: unknown) => Caibird.dp.PromiseOrSelf<void>) | undefined;
export const setOnAppError = (fn: (err: unknown) => Caibird.dp.PromiseOrSelf<void>) => onAppError = fn;
            ...
            if (!error) {
                ...
                try {
                    ...
                    if (errName && errJson) {
                        ...
                        error = new (ErrClass as unknown as Caibird.dp.Class)(...errJson.args) as Error;
                    ...
                    try {
                        error = new (window as unknown as Caibird.dp.Obj<Caibird.dp.Class>)[errName ?? '']() as Error;
                    } catch {
                        error = new Error();
                    }
